...
local function SetCharacterLimit(self, limit)
    self.unity_UITMPInput.characterLimit = limit
end
...
local function OnDestroy(self)
    ...
    if self.__onSelect ~= nil then
        self.unity_UITMPInput.onSelect:RemoveListener(self.__onSelect)
    end
...
local function SetOnSelect(self, action)
    if action then
        if self.__onSelect then
            self.unity_UITMPInput.onSelect:RemoveListener(self.__onSelect)
        end
        self.__onSelect = action
        self.unity_UITMPInput.onSelect:AddListener(self.__onSelect)
    elseif self.__onSelect then
        self.unity_UITMPInput.onSelect:RemoveListener(self.__onSelect)
        self.__onSelect = nil
    end
end
...
UITMPInput.SetOnSelect = SetOnSelect
...
UITMPInput.SetCharacterLimit = SetCharacterLimit
