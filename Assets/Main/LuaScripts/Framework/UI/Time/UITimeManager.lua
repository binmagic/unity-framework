--[[
-- [INPUT]: 依赖 luasocket 的 gettime 取本地时钟、Singleton 基类、Localization 多语言格式串、LuaEntry.Player/GlobalData 的开服与今日零点数据、DataCenter.SeasonDataManager 赛季信息
-- [OUTPUT]: 对外提供 UITimeManager 单例，维护服务器时间偏移（UpdateServerMsDeltaTime/GetServerTime），并暴露大量时间戳格式化、倒计时字符串、周期计算（GetWeekdayIndex/GetNextWeekDay/IsSameDay 等）工具函数
-- [POS]: UI 层的时间服务中枢，区分服务器无时区时间与本地时区时间；被 UI 计时、邮件、聊天、活动倒计时等广泛复用，独立于窗口生命周期
