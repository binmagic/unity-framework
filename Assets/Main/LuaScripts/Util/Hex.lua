...
--[[
-- [INPUT]: 纯 Lua + math 库实现,无外部模块依赖;采用顶点朝上、奇数行右移的偏移坐标约定
-- [OUTPUT]: 对外提供 Hex 表(Offset<->Cube 坐标转换、CubeRound 取整、TileDistance 格距、半径常量)
-- [POS]: Util 的六边形网格数学库(移植自 hex.cs),为六角地图的坐标换算与寻路距离提供纯计算支撑,不涉及渲染与数据层
