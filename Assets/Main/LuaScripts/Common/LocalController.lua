...
function LocalController:GetABTestTableName(tableName)
	-- AB 测试依赖 LuaEntry.DataConfig 和 LuaEntry.Player，本项目裁剪后两者都不存在。
	-- 缺任一就直接返回原表名跳过 AB 分流，否则 index nil 会让所有配置表读取失败。
	if LuaEntry == nil or LuaEntry.DataConfig == nil or LuaEntry.Player == nil then
		return tableName
	end
