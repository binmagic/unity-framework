...
local OptimizeUtils = CS.xLuaOptiUtils
...
function Quaternion.Euler(x, y, z)
	...
	local q = Quaternion.New(OptimizeUtils.Quaternion_EulerToQuat(x,y,z))
	return q
end

function Quaternion.EulerToQuat(x, y, z)
	local qx,qy,qz,qw = OptimizeUtils.Quaternion_EulerToQuat(x,y,z)
	return qx,qy,qz,qw
end
...
function Quaternion:SetEuler(x, y, z)		
	...
	self.x, self.y, self.z, self.w = Quaternion.EulerToQuat(x, y, z)
...
function Quaternion.FastLookRotation(forward, up)
	up = up or _up
	
	--直接调用c#来做计算
	local x, y, z, w = OptimizeUtils.Quaternion_LookRotation(forward.x, forward.y, forward.z, up.x, up.y, up.z)
	return x, y, z, w
end
...
function Quaternion.RotateTowards(from, to, maxDegreesDelta)
	do
		local x, y, z, w = OptimizeUtils.Quaternion_RotateTowards(from.x, from.y, from.z, from.w, to.x, to.y, to.z, to.w, maxDegreesDelta)
		return _new(x, y, z, w)
	end
...
function Quaternion:ToEulerAngles()
	do
		local x, y, z = OptimizeUtils.Quaternion_ToEulerAngles(self.x, self.y, self.z, self.w)
...
function Quaternion.MulVec3(self, point)
	do
		local x, y, z = self:MulVec3XYZ(point)
		return Vector3.New(x, y, z)
	end
...
function Quaternion.MulVec3XYZ(self, point)
	local x, y, z = OptimizeUtils.Quaternion_MulVec3(self.x, self.y, self.z, self.w, point.x, point.y, point.z)
	return x, y, z
end
...
Quaternion.__mul = function(lhs, rhs)
	...
	--elseif Quaternion == getmetatable(rhs) then --这里 两个源表 == 会有问题 
	else
		return Quaternion.New((((lhs.w * rhs.x) + (lhst.x * rhs.w)) + (lhs.y * rhs.z)) - (lhs.z * rhs.y), (((lhs.w * rhs.y) + (lhs.y * rhs.w)) + (lhs.z * rhs.x)) - (lhs.x * rhs.z), (((lhs.w * rhs.z) + (lhs.z * rhs.w)) + (lhs.x * rhs.y)) - (lhs.y * rhs.x), (((lhs.w * rhs.w) - (lhs.x * rhs.x)) - (lhs.y * rhs.y)) - (lhs.z * rhs.z))
	end
...
Quaternion.RotateZ0 = Quaternion.Euler(0, 0, 0)
Quaternion.RotateZ90 = Quaternion.Euler(0, 0, 90)
Quaternion.RotateZ180 = Quaternion.Euler(0, 0, 180)
Quaternion.RotateZ270 = Quaternion.Euler(0, 0, -90)
