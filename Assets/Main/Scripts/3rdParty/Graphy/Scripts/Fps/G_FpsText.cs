...
namespace Tayx.Graphy.Fps
{
    public class G_FpsText : MonoBehaviour
    {
        ...
        [SerializeField] private    Text            m_cpuTimeText        = null;
        [SerializeField] private    Text            m_gpuTimeText        = null;
        ...
        private void Update()
        {
            ...
            if (m_deltaTime > 1f / m_updateRate)
            {
                ...
                // Update cpu time & gpu time
                FrameTimingManager.GetLatestTimings(1, frameTimings);
                if (frameTimings.Length > 0)
                {
                    m_cpuTimeText.text = ((float)frameTimings[0].cpuFrameTime).ToStringNonAlloc(m_msStringFormat);
                    m_gpuTimeText.text = ((float)frameTimings[0].gpuFrameTime).ToStringNonAlloc(m_msStringFormat);
                }
