    ...
    public class LuaEnv : IDisposable
    {
        ...
        public LuaEnv()
        {
...
#endif
            {
                ...
                XLua.LuaDLL.Lua.LoadStringExt(rawL);
                try
                {
                    XLua.LuaDLL.Lua.LoadLuaCNativeCode(rawL);
                    // XLua.LuaDLL.Lua.LoadLuaMPackCode(rawL);
                }
                catch (Exception e)
                {
                ...
                UnityEngine.Debug.Log("LuaEnv init ok!");
            }
        }
        ...
        public virtual void Dispose(bool dispose)
        {
            ...
            lock (luaEnvLock)
            {
                ...
                {
                    #if DEBUG
                    DoString("local util = require 'xlua.util' util.print_func_ref_by_csharp()");
                    #endif
                    // throw new InvalidOperationException($"try to dispose a LuaEnv with C# callback! \n {translator.GetDelegateBridgeCount()} Delegate not released");
                }
        ...
        public void ThrowExceptionFromError(int oldTop)
        {
            ...
            lock (luaEnvLock)
            {
                ...
                PostEventLog.LogToFeishu(err.ToString(), 1);
        ...
        private string init_xlua = @" 
            ...
            function metatable:__newindex(key, value)
                ...
                error('Error: No such type: ' .. key, 2)
            end
        ...
        //zlh note: lua文件解析GC优化：这里加个简单的结构来传递lua文件相关的信息 兼容新老方式
        //1.老的方式通过optionalBuf传递 来源是TextAsset.bytes获取的 会多一份内存拷贝
        //2.新的方式通过nativeBufPtr及dataSize传递， ptr是通过反射获取的TextAsset对应的内存地址
        public struct LuaFileInfo
        {
            ...
            public VEngine.Asset assetRequest;
            public Action<VEngine.Asset> checkIfReleaseAsset;
        }
        
        public delegate LuaFileInfo CustomLoader(ref string filepath);
