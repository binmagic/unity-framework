...
namespace XLua
{
	...
	public static partial class Utils
	{
		...
		// 加几个全局变量，用来避免GC每次的分配
		private static List<PropertyInfo> items = new List<PropertyInfo>(4);
		private static PropertyInfo[] EmptyPropertyArray = new PropertyInfo[0];
		private static Dictionary<MethodKey, List<MemberInfo>> pending_methods = new Dictionary<MethodKey, List<MemberInfo>>();
			...
			int obj_field, int obj_getter, int obj_setter, int obj_meta, out LuaCSFunction item_getter, out LuaCSFunction item_setter, BindingFlags access)
		{
			...
			var item_array = items.Count > 0 ? items.ToArray() : EmptyPropertyArray;
			...
			for (int i = 0; i < methods.Length; ++i)
			{
				...
				else if (method_name.StartsWith("get_") && method.IsSpecialName && method.GetParameters().Length != 1) // getter of property
				{
					...
					translator.PushFixCSFunction(L, translator.methodWrapsCache._GenMethodWrap(method.DeclaringType, prop_name, method ).Call);
			...
			foreach (var kv in pending_methods)
			{
				...
				if (kv.Key.Name.StartsWith("op_")) // 操作符
				{
					...
					{
						translator.PushFixCSFunction(L,
							new LuaCSFunction(translator.methodWrapsCache._GenMethodWrap(type, kv.Key.Name, kv.Value[0]).Call));
					}
		...
		static List<string> getPathOfType( Type type, List<string> path)
		{
			path.Clear();
#if		false	
			...
			string class_name = type.ToString().Substring(type.Namespace?.Length + 1 ?? 0);
...
#else
			var type2Str = type.ToString();
			path.AddRange(type2Str.Split(SplitChars));
#endif			
			return path;
		}
		...
		public static void LoadCSTable(RealStatePtr L, Type type)
		{
            ...
            getPathOfType(type, TempList);

			for (int i = 0; i < TempList.Count; ++i)
			{
				LuaAPI.xlua_pushasciistring(L, TempList[i]);
				...
				if (!LuaAPI.lua_istable(L, -1) && i < TempList.Count - 1)
				{
					LuaAPI.lua_settop(L, oldTop);
					LuaAPI.lua_pushnil(L);
					return;
		...
		private static readonly List<string> TempList = new List<string>(8);
		...
		public static void SetCSTable(RealStatePtr L, Type type, int cls_table)
		{
			...
			LuaAPI.xlua_pushasciistring(L, TempList[TempList.Count - 1]);
