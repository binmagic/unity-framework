...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        public bool TryDelayWrapLoader(RealStatePtr L, Type type)
        {
            ...
            {
// #if !FINAL_RELEASE
//                 UnityEngine.Debug.LogFormat(" --#XLUA#-- wrap loader: {0}", type.FullName);
// #endif
...
#else
                //UnityEngine.Debug.LogFormat("#LSY# Delay Type : {0}", type.FullName);
#if !FINAL_RELEASE
               // GameEntry.Lua.LuaTraceback();
                UnityEngine.Debug.LogFormat(" --#XLUA#-- Delay Type : {0}", type.FullName);
#endif
		...
		{
            assemblies = new List<Assembly>(80);
            ...
            UnityEngine.Debug.LogFormat("ObjectTranslator assemblies = {0}", assemblies.Count());
        ...
        public object CreateDelegateBridge(RealStatePtr L, Type delegateType, int idx)
        {
            ...
            try
            {
                // 编辑器模式下使用反射创建，因为有可能用户没有生成xlua；
                // DelegateBridge_Wrap的构造函数会被裁剪，Unity的裁剪这个比较坑。所以这里手动调用一下构造函数
#if UNITY_EDITOR
                bridge = Activator.CreateInstance(delegate_birdge_type, new object[] { reference, luaEnv }) as DelegateBridgeBase;
#else               
                bridge = new XLua.DelegateBridge_Wrap(reference, luaEnv);
#endif
            }
            catch(Exception e)
            {
