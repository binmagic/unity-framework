    ...
    public static class Generator
    {
        ...
        static Generator()
        {
            ...
            luaenv.AddLoader((ref string filepath) =>
            {
                ...
                {
                    var buf = Encoding.UTF8.GetBytes(templateRef.TemplateCommon.text);
        ...
        static bool isMethodInBlackList(MethodBase mb)
        {
            ...
            foreach (var exclude in BlackList)
            {
                string t = mb.DeclaringType.ToString();
                if (t == exclude[0] && mb.Name == exclude[1])
                {
                    // 没必要判断参数，只判断类型名和函数名
                    return true;
                ...
                // 类型包含也处理掉
                if (t.Contains(exclude[0]))
                {
                    if (mb.Name == exclude[1])
                    {
                        return true;
                    }
                }
...
#pragma warning restore 618
        ...
        public static void GenAll()
        {
            ...
            // 删除多余的meta
            var files2 = Directory.GetFiles(GeneratorConfig.common_path, "*.meta");
            foreach (var file in files2)
            {
                string csfile = file.Remove(file.IndexOf(".meta"));
