    ...
    public partial class Lua
	{
        ...
        public static string lua_tostring(IntPtr L, int index)
		{
            ...
            if (str != IntPtr.Zero)
			{
				...
				try
				{
					int slen = strlen.ToInt32();
					if (slen > 0)
					{
						ret = Marshal.PtrToStringAnsi(str, slen);
...
// 						}
// #endif					
// 						Debug.LogErrorFormat("marshal string: {0}", ret);
					}
					else
					{
						// 如果字符串是空串的话，就不要Marshal一个新的空串了。能省则省。。
						ret = string.Empty;
					}
				}
				catch (Exception e)
				{
					// 因为我们的消息中，有protobuf，里面会有一些二进制字符串，是无效字符的。
// #if UNITY_EDITOR
// 					if (GameEntry.Lua != null)
// 					{
// 						GameEntry.Lua.LuaTraceback();
// 					}
// #endif					
					Debug.LogFormat("marshal error: {0}", e.Message );
				}
