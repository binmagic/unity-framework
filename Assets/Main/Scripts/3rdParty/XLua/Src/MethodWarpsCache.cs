...
namespace XLua
{
    ...
    {
        // 这里加一些处理GC的代码，虽然理论上这种优化空间不大，但是顺手修改一下也无妨
        private static int[] EmptyIntArray = new int[0];
        private static ObjectCheck[] EmptyObjectCheckArray = new ObjectCheck[0];
        private static ObjectCast[] EmptyObjectCastArray = new ObjectCast[0];
        private static bool[] EmptyBoolArray = new bool[0];
        private static object[] EmptyObjectArray = new object[0];
        
        private static List<int> inPosList = new List<int>();
        private static List<int> outPosList = new List<int>();
        
        private static List<ObjectCheck> paramsChecks = new List<ObjectCheck>();
        private static List<ObjectCast> paramsCasts = new List<ObjectCast>();
        private static List<bool> isOptionalList = new List<bool>();
        private static List<object> defaultValueList = new List<object>();
        ...
        public void Init(ObjectCheckers objCheckers, ObjectCasters objCasters)
        {
            ...
            refPos = paramInfos.Length > 0 ? new int[paramInfos.Length] : EmptyIntArray;
            ...
            // 如果有相应数据的话，就分配一个，否则使用全局的空数组！
            checkArray = paramsChecks.Count > 0 ? paramsChecks.ToArray() : EmptyObjectCheckArray;
            castArray = paramsCasts.Count > 0 ? paramsCasts.ToArray() : EmptyObjectCastArray;
            inPosArray = inPosList.Count > 0 ? inPosList.ToArray() : EmptyIntArray;
            outPosArray = outPosList.Count > 0 ? outPosList.ToArray() : EmptyIntArray;
            isOptionalArray = isOptionalList.Count > 0 ? isOptionalList.ToArray() : EmptyBoolArray;
            defaultValueArray = defaultValueList.Count > 0 ? defaultValueList.ToArray() : EmptyObjectArray;
            ...
            args = paramInfos.Length > 0 ? new object[paramInfos.Length] : EmptyObjectArray;
        ...
        public int Call(RealStatePtr L)
        {
            try
            {
                ...
                // 这个地方是我们简单的修改了一下xlua的代码，如果没有arg的话，就置为null。否则会有一个GC。目前我们每帧都会调用lua函数
                // if (toInvoke.IsConstructor)
                // {
                //     ret = ((ConstructorInfo) method).Invoke(args);
                // }
                // else
                // {
                //     if (args.Length > 0)
                //     {
                //         ret = method.Invoke(targetNeeded ? target : null, args);
                //     }
                //     else
                //     {
                //         ret = method.Invoke(targetNeeded ? target : null, null);
                //     }
                // }
    ...
    public class MethodWrap
    {
        ...
        private List<OverloadMethodWrap> overloads = null;// new List<OverloadMethodWrap>();
    ...
    public class MethodWrapsCache
    {
        ...
        { 
            List<OverloadMethodWrap> overloads = new List<OverloadMethodWrap>(methodBases.Count());
        ...
        // 加一个处理单一methodBase的函数，大部分都是单一函数
