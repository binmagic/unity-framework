...
namespace XLua
{
    ...
    public partial class StaticLuaCallbacks
    {
        ...
        internal static int Print(RealStatePtr L)
        {
            ...
            {
                // 不写LOG的话，直接返回
                int level = Log.GetWriteLogLevel();
                if (level <= 0)
                {
                    return 0;
                }
        ...
        internal static int LoadFromCustomLoaders(RealStatePtr L)
        {
            try
            {
                ...
                foreach (var loader in self.customLoaders)
                {
                    ...
                    var luaFileInfo = loader(ref real_file_path);
                    var bytes = luaFileInfo.optionalBuf;
                    if (bytes != null)
                    {
                        if (LuaAPI.xluaL_loadbuffer(L, bytes, bytes.Length, "@" + real_file_path) != 0)
                        {
                            luaFileInfo.checkIfReleaseAsset?.Invoke(luaFileInfo.assetRequest);
                    ...
                    if (luaFileInfo.nativeBufPtr != IntPtr.Zero)
                    {
                        //UnityEngine.Debug.LogError($"\n#zlh# LoadFromCustomLoaders fileName:{filename}, nativeBufPtr:{luaFileInfo.nativeBufPtr} dataSize:{luaFileInfo.dataSize}"); 
                        if (LuaAPI.xluaL_loadbuffer(L, luaFileInfo.nativeBufPtr, (int)luaFileInfo.dataSize, "@" + real_file_path) != 0)
                        {
                            luaFileInfo.checkIfReleaseAsset?.Invoke(luaFileInfo.assetRequest);
                    ...
                    // editor模式下，直接读取原始文件。读取代码写在C中
                    if (luaFileInfo.filepath != null)
                    {
                        if (LuaAPI.xluaL_loadfile(L, luaFileInfo.filepath, "@" + real_file_path) != 0)
                        {
                            return LuaAPI.luaL_error(L, String.Format("error loading module {0} from CustomLoader, {1}",
                                LuaAPI.lua_tostring(L, 1), LuaAPI.lua_tostring(L, -1)));
                        }
