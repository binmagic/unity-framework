...
namespace Sfs2X.Protocol.Serialization
{
  public class DefaultSFSDataSerializer : ISFSDataSerializer
  {
    ...
    private ISFSObject DecodeSFSObject(ByteArray buffer)
    {
      ...
      try
      {
        for (int index = 0; index < num2; ++index)
        {
          ...
          SFSDataWrapper val = DecodeObject(buffer);
          if (val == null)
    ...
    private ISFSArray DecodeSFSArray(ByteArray buffer)
    {
      ...
      try
      {
        for (int index = 0; index < num; ++index)
        {
          SFSDataWrapper val = DecodeObject(buffer);
          if (val == null)
            throw new SFSCodecError("Could not decode SFSArray item at index: " + index);
          sfsArray.Add(val);
        }
      }
      catch (SFSCodecError ex)
    ...
    private SFSDataWrapper DecodeObject(ByteArray buffer)
    {
      ...
      switch (int32)
      {
        case SFSDataType.NULL:
          sfsDataWrapper = BinDecode_NULL(buffer);
          break;
        case SFSDataType.BOOL:
          sfsDataWrapper = BinDecode_BOOL(buffer);
          break;
        case SFSDataType.BYTE:
          sfsDataWrapper = BinDecode_BYTE(buffer);
          break;
        case SFSDataType.SHORT:
          sfsDataWrapper = BinDecode_SHORT(buffer);
          break;
        case SFSDataType.INT:
          sfsDataWrapper = BinDecode_INT(buffer);
          break;
        case SFSDataType.LONG:
          sfsDataWrapper = BinDecode_LONG(buffer);
          break;
        case SFSDataType.FLOAT:
          sfsDataWrapper = BinDecode_FLOAT(buffer);
          break;
        case SFSDataType.DOUBLE:
          sfsDataWrapper = BinDecode_DOUBLE(buffer);
          break;
        case SFSDataType.UTF_STRING:
          sfsDataWrapper = BinDecode_UTF_STRING(buffer);
          break;
        case SFSDataType.BOOL_ARRAY:
          sfsDataWrapper = BinDecode_BOOL_ARRAY(buffer);
          break;
        case SFSDataType.BYTE_ARRAY:
          sfsDataWrapper = BinDecode_BYTE_ARRAY(buffer);
          break;
        case SFSDataType.SHORT_ARRAY:
          sfsDataWrapper = BinDecode_SHORT_ARRAY(buffer);
          break;
        case SFSDataType.INT_ARRAY:
          sfsDataWrapper = BinDecode_INT_ARRAY(buffer);
          break;
        case SFSDataType.LONG_ARRAY:
          sfsDataWrapper = BinDecode_LONG_ARRAY(buffer);
          break;
        case SFSDataType.FLOAT_ARRAY:
          sfsDataWrapper = BinDecode_FLOAT_ARRAY(buffer);
          break;
        case SFSDataType.DOUBLE_ARRAY:
          sfsDataWrapper = BinDecode_DOUBLE_ARRAY(buffer);
          break;
        case SFSDataType.UTF_STRING_ARRAY:
          sfsDataWrapper = BinDecode_UTF_STRING_ARRAY(buffer);
          break;
        case SFSDataType.SFS_ARRAY:
          --buffer.Position;
          sfsDataWrapper = new SFSDataWrapper(SFSDataType.SFS_ARRAY, DecodeSFSArray(buffer));
          break;
        case SFSDataType.SFS_OBJECT:
          --buffer.Position;
          ISFSObject sfsObj = DecodeSFSObject(buffer);
          byte num = Convert.ToByte(18);
          object data = sfsObj;
          if (sfsObj.ContainsKey(CLASS_MARKER_KEY) && sfsObj.ContainsKey(CLASS_FIELDS_KEY))
          {
            num = Convert.ToByte(19);
            data = Sfs2Cs(sfsObj);
          }
          sfsDataWrapper = new SFSDataWrapper((SFSDataType)num, data);
          break;
        case SFSDataType.TEXT:
          sfsDataWrapper = BinDecode_TEXT(buffer);
          break;
        default:
    ...
    private SFSDataWrapper BinDecode_BOOL_ARRAY(ByteArray buffer)
    {
      ...
      bool[] flagArray = new bool[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        flagArray[index] = buffer.ReadBool();
      return new SFSDataWrapper(SFSDataType.BOOL_ARRAY, flagArray);
    }
    ...
    private SFSDataWrapper BinDecode_BYTE_ARRAY(ByteArray buffer)
    {
      ...
      var tempBuf = buffer.ReadBytes(count);
      var wrapper = new SFSDataWrapper(SFSDataType.BYTE_ARRAY, new ByteArray(tempBuf));
    ...
    {
      int typedArraySize = GetTypedArraySize(buffer);
      short[] numArray = new short[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        numArray[index] = buffer.ReadShort();
      return new SFSDataWrapper(SFSDataType.SHORT_ARRAY, numArray);
    }

    private SFSDataWrapper BinDecode_INT_ARRAY(ByteArray buffer)
    {
      int typedArraySize = GetTypedArraySize(buffer);
      int[] numArray = new int[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        numArray[index] = buffer.ReadInt();
      return new SFSDataWrapper(SFSDataType.INT_ARRAY, numArray);
    }

    private SFSDataWrapper BinDecode_LONG_ARRAY(ByteArray buffer)
    {
      int typedArraySize = GetTypedArraySize(buffer);
      long[] numArray = new long[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        numArray[index] = buffer.ReadLong();
      return new SFSDataWrapper(SFSDataType.LONG_ARRAY, numArray);
    }

    private SFSDataWrapper BinDecode_FLOAT_ARRAY(ByteArray buffer)
    {
      int typedArraySize = GetTypedArraySize(buffer);
      float[] numArray = new float[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        numArray[index] = buffer.ReadFloat();
      return new SFSDataWrapper(SFSDataType.FLOAT_ARRAY, numArray);
    }

    private SFSDataWrapper BinDecode_DOUBLE_ARRAY(ByteArray buffer)
    {
      int typedArraySize = GetTypedArraySize(buffer);
      double[] numArray = new double[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        numArray[index] = buffer.ReadDouble();
      return new SFSDataWrapper(SFSDataType.DOUBLE_ARRAY, numArray);
    }

    private SFSDataWrapper BinDecode_UTF_STRING_ARRAY(ByteArray buffer)
    {
      int typedArraySize = GetTypedArraySize(buffer);
      string[] strArray = new string[typedArraySize];
      for (int index = 0; index < typedArraySize; ++index)
        strArray[index] = buffer.ReadUTF();
      return new SFSDataWrapper(SFSDataType.UTF_STRING_ARRAY, strArray);
    }
