...
namespace SuperScrollView
{
    ...
    public class ItemPool
    {
        ...
        Queue<LoopListViewItem2> mTmpPooledItemList = new Queue<LoopListViewItem2>();
        Queue<LoopListViewItem2> mPooledItemList = new Queue<LoopListViewItem2>();
        ...
        public LoopListViewItem2 GetItem()
        {
            ...
            {
                tItem = mTmpPooledItemList.Dequeue();
        ...
        public void DestroyAllItem()
        {
            ...
            {
                var item = mPooledItemList.Dequeue();
        ...
        void RecycleItemReal(LoopListViewItem2 item)
        {
            ...
            mPooledItemList.Enqueue(item);
        }
        public void RecycleItem(LoopListViewItem2 item)
        {
            mTmpPooledItemList.Enqueue(item);
        }
        public void ClearTmpRecycledItem()
        {
            ...
            for(int i = 0;i<count;++i)
            {
                RecycleItemReal(mTmpPooledItemList.Dequeue());
            }
...
//                if(mScrollRectTransform.anchorMax == Vector2.one && mScrollRectTransform.anchorMin == Vector2.zero)
                ...
                //取列表中第一个item左上角和左下角相对于ViewPort的局部坐标
                Vector3 topPos0 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[1]);
                Vector3 downPos0 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[0]);
                
                //当第一个元素超出屏幕上方一定距离后回收，并刷新Content尺寸 以及更新所有item的pos
                if (tViewItem0.ItemCreatedCheckFrameCount != mListUpdateCheckFrameCount
                ...
                //去取列表中最后一个item左上角和左下角相对于ViewPort的局部坐标
                LoopListViewItem2 tViewItem1 = mItemList[mItemList.Count - 1];
                tViewItem1.CachedRectTransform.GetWorldCorners(mItemWorldCorners);
                Vector3 topPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[1]);
                Vector3 downPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[0]);
                
                //当最后一个元素超出屏幕下方一定距离后回收，并刷新Content尺寸 以及更新所有item的pos
                if (tViewItem1.ItemCreatedCheckFrameCount != mListUpdateCheckFrameCount
                ...
                //当最后一个元素和屏幕下方的距离小于一定距离，则尝试生成新的item
                ...
                //当第一个元素和屏幕上方的距离小于一定距离，则尝试生成新的item
