...
namespace RenderHeads.Media.AVProVideo
{
	...
	public partial class MediaPlayer : MonoBehaviour
	{
		...
		private void SetLoadOptions()
		{
			...
			{
				if (_baseMediaPlayer is WindowsMediaPlayer) { _baseMediaPlayer = CreateMediaPlayer(); }
				((WindowsRtMediaPlayer)_baseMediaPlayer).SetOptions(_optionsWindows);
			}
			else
			#endif
			{
				if (_baseMediaPlayer is WindowsRtMediaPlayer) { _baseMediaPlayer = CreateMediaPlayer(); }
